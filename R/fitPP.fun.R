...
function(covariates=NULL,beta,posE=NULL,inddat=NULL,POTob=NULL,namcovariates=NULL,n=NULL,
	tind='TRUE', tim=NULL, modCI='TRUE',CIty='Delta', clevel=0.95,
	tit='', modSim='FALSE',dplot=TRUE,xlegend='topleft')
{
	...
	{
		dev.new()
		if (modCI==TRUE) limy<-c(min(LIlambda, na.rm=TRUE), max(UIlambda, na.rm=TRUE))
		else limy<-c(min(lambdafit, na.rm=TRUE), max(lambdafit, na.rm=TRUE))
		plot(tim,lambdafit, ty='n',ylab='intensity', xlab='time', ylim=limy)
		if (modCI==TRUE)
		{
